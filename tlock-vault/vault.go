...
func (vault *TLockVault) AddFolder(name string) {
	vault.Data.Folders = append(vault.Data.Folders, Folder{Name: name})
...
func (vault *TLockVault) GetTokens(folder string) []Token {
...
func (vault *TLockVault) AddToken(folder, uri string) {
	...
	vault.Data.Folders[folder_index].Uris = append(vault.Data.Folders[folder_index].Uris, Token{URI: uri, UsageCounter: 0})
...
func (vault TLockVault) find_folder(name string) int {
	return slices.IndexFunc(vault.Data.Folders, func(item Folder) bool { return item.Name == name })
}

// Returns the index of the folder based on the name
func (vault TLockVault) find_token(folder_index int, token string) int {
	return slices.IndexFunc(vault.Data.Folders[folder_index].Uris, func(item Token) bool { return item.URI == token })
}
